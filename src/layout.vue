...
import TableProCell from './table-pro-cell.vue';
...
// Capture SDK-injected stores once during setup so helpers invoked from
// template/event handlers (which run outside setup context) don't hit the
// "[useStores]: The stores could not be found." inject error.
const { useFieldsStore: storeFields, useRelationsStore: storeRelations } = useStores();
const fieldsStore = storeFields();
const relationsStore = storeRelations();
...
function addField(fieldKey: string) {
	if (fieldsWritable.value.includes(fieldKey))
		return;
...
function onFieldListAdd(fieldKeys: string[] | string) {
	const keys = Array.isArray(fieldKeys) ? fieldKeys : [fieldKeys];
	for (const key of keys) {
		if (!fieldsWritable.value.includes(key))
			fieldsWritable.value = [...fieldsWritable.value, key];
	}
	addFieldMenuOpen.value = false;
}

function onRelationalCellClick(header: any, item: any, event: MouseEvent) {
	// Open related item in the side drawer (standard Directus navigation)
	// when a relational cell is clicked. For top-level M2O fields the target
	// is the related row referenced by the column; for nested paths we walk
	// as far down as the data goes and route to that record.
	if (!header?.field)
		return;
	const drillTarget = resolveDrillTarget(header, item);
	if (!drillTarget)
		return;

	event?.stopPropagation?.();
	router.push(`/content/${drillTarget.collection}/${drillTarget.id}`);
}
...
function resolveDrillTarget(header: any, item: any): DrillTarget | null {
	const pathSegments = (header.value as string).split('.');
	const parentCollection = header.field?.collection as string | undefined;
	if (!parentCollection)
		return null;

	let currentCollection = parentCollection;
	let currentValue: any = item;
	let lastResolved: DrillTarget | null = null;

	for (let i = 0; i < pathSegments.length; i++) {
		const segment = pathSegments[i]!;
		const fieldInfo = fieldsStore.getField(currentCollection, segment);
		if (!fieldInfo)
			break;

		const relatedCollection = findRelatedCollection(
			currentCollection,
			segment,
		);
		...
		const rawValue = currentValue?.[segment];
		if (rawValue === undefined || rawValue === null) {
			break;
		}

		const relatedPk = extractPk(rawValue);
		if (relatedPk === null) {
			break;
		}

		lastResolved = { collection: relatedCollection, id: relatedPk };
		currentCollection = relatedCollection;
		currentValue = typeof rawValue === 'object' ? rawValue : null;
	}

	return lastResolved;
}

function findRelatedCollection(
	collection: string,
	field: string,
): string | null {
	const relations = (relationsStore as any).relations ?? [];
...
function extractPk(value: any): string | number | null {
...
function isRelationalHeader(header: any): boolean {
	if (!header?.field)
		return false;
	const key = header.value as string;
	if (key.includes('.'))
		return true;
	const special: string[] = header.field?.meta?.special ?? [];
	if (special.some((s: string) => ['m2o', 'file', 'files', 'm2a', 'translations', 'o2m', 'm2m'].includes(s)))
		return true;
	const parentCollection = header.field?.collection as string | undefined;
	if (!parentCollection)
		return false;
	return !!findRelatedCollection(parentCollection, key);
}
...
<template>
	<div class="layout-table-pro">
		<v-table
			v-if="loading || (itemCount && itemCount > 0 && !error)" ref="table" v-model="selectionWritable"
		...
		>
			<template #header-context-menu="{ header }">
				<v-list>
					...
					<v-list-item
						:active="header.align === 'left'" clickable
					...
					<v-list-item
						:active="header.align === 'center'" clickable
					...
					<v-list-item
						:active="header.align === 'right'" clickable
			...
			<template #header-append>
				...
				>
					<template #activator="{ toggle, active }">
						<v-icon
							v-tooltip="t('add_field')" class="add-field" name="add" :class="{ active }" clickable
							@click="toggle"
						/>
					</template>
			...
			<template v-for="header, index in tableHeaders" :key="header.value" #[`item.${header.value}`]="{ item }">
				<TableProCell
					:column="index" :item="item" :field-key="header.value" :field-edits="edits[item[primaryKeyField?.field]]?.[header.value]"
					:readonly="!header.editable"
					@leave-cell="autoSaveEdits"
				>
					<template #display="{ displayItem }">
						<component
							:is="isRelationalHeader(header) ? 'a' : 'div'"
							class="display-wrapper"
							:class="{ 'is-link': isRelationalHeader(header) }"
							@click="isRelationalHeader(header) && onRelationalCellClick(header, displayItem, $event)"
						>
							<render-display
								:value="getFromAliasedItem(displayItem, header.value)"
					...
					<template #interface>
						<v-form
							v-model="editsWritable[item[primaryKeyField?.field]]" :fields="[header.field]"
							:initial-values="item" :loading="loading"
			...
			<template #item-append="{ item }">
				<v-button
					:to="`/content/${collection}/${item[primaryKeyField?.field]}`" :tooltip="t('edit_item')" icon
					secondary x-small
				>
					<v-icon class="small" name="edit" />
				</v-button>
			</template>
			...
			<template #footer>
				<div class="footer">
					<div class="pagination">
						<v-pagination
							v-if="totalPages > 1" :length="totalPages" :total-visible="7" show-first-last
					...
					<div v-if="loading === false && (items.length >= 25 || limit < 25)" class="per-page">
						...
						<v-select
							:model-value="`${limit}`" :items="pageSizes" inline
	...
	<v-dialog v-model="confirmLeave" @esc="confirmLeave = false">
		<v-card>
			...
			<v-card-actions>
				...
				<v-button @click="confirmLeave = false">
					{{ t('keep_editing') }}
				</v-button>
			</v-card-actions>
		</v-card>
