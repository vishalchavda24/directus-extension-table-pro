...
function useCellNavigation(preventNavigation: Ref<boolean>, enterCell: any, leaveCell: any) {
	...
	function cellNavigation(e: KeyboardEvent, { vertical = false, next = false }) {
		...
		const cell = vertical
			? parentSibling?.querySelectorAll('.table-pro-cell')?.[props.column]
			: parentSibling?.querySelector('.table-pro-cell');
...
<template>
	<div
		ref="target" :tabindex="editMode && tabPress ? null : 0" class="table-pro-cell"
		:class="{ 'edit-mode': editMode, 'read-only': readonly }" @dblclick="enterCell"
	>
